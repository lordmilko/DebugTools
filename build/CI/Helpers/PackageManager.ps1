...
function PackageManager
{
    ...
    param(
        ...
        [Parameter(Mandatory = $true, ParameterSetName = "ProjectLocation")]
...
class PackageManager
{
    ...
    [void]InstallPackageSource($language, $exists, $register, $unregister)
    {
        ...
        if(Test-Path (PackageManager -PackageLocation))
        {
            Write-LogError "`t`t`tRemoving temp package folder left over from previous run..."
            Remove-Item (PackageManager -PackageLocation) -Recurse -Force
        }

        Write-LogInfo "`t`tInstalling temp project folder"

        if(Test-Path (PackageManager -ProjectLocation))
        {
            Write-LogError "`t`t`tRemoving project folder left over from previous run..."

            Remove-Item (PackageManager -ProjectLocation) -Recurse -Force
        }

        Write-LogInfo "`t`t`tCreating project folder"
        New-Item -ItemType Directory (PackageManager -ProjectLocation) | Out-Null
    }
    ...
    [void]UninstallPackageSource($language, $unregister)
    {
        ...
        Write-LogInfo "`t`tUninstalling temp package folder"
        Remove-Item (PackageManager -PackageLocation) -Recurse -Force -ErrorAction SilentlyContinue

        Write-LogInfo "`t`tUninstalling temp project folder"
        Remove-Item (PackageManager -ProjectLocation) -Recurse -Force
    }
